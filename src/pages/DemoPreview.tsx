...
export default function DemoPreview() {
    ...
    const fetchUserName = async () => {
      ...
      if (user) {
        const { data: profile } = await supabase
          .from(demoRole === "tutor" ? "tutor_profiles" : "learner_profiles")
          .select("full_name")
          .eq("id", user.id)
          .single();
  ...
  // Dummy session data with real user name
  const demoSessionData = {
    subject: "Dummy Session",
    scheduled_at: new Date().toISOString(),
    tutor_profiles: {
      full_name: demoRole === "tutor" ? (userName || "You") : "Dummy Tutor"
    },
    learner_profiles: {
      full_name: demoRole === "learner" ? (userName || "You") : "Dummy Learner"
    }
  };
  ...
  const handleContinue = () => {
    // Pass device IDs along with camera/mic state
    ...
    // Learners go through admit page, tutors go directly to session
    if (demoRole === "learner") {
      navigate(`/demo-admit?${params.toString()}`);
    } else {
  ...
  const handleCancel = () => {
    localStream?.getTracks().forEach((track) => track.stop());
    navigate(demoRole === "tutor" ? "/tutor/dashboard" : "/learner/dashboard");
  };

  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
      <div className="bg-background max-w-4xl w-[calc(100%-2rem)] max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl">
        {/* Header */}
        <div className="text-center p-6 border-b">
          <h1 className="text-2xl font-semibold break-words">{demoSessionData.subject}</h1>
        </div>
        ...
        <div className="p-6">
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-muted-foreground break-words">{demoSessionData.subject}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      {new Date(demoSessionData.scheduled_at).toLocaleDateString('en-US', {
                        weekday: 'short',
                        month: 'short',
                        day: 'numeric'
                      })}
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      {new Date(demoSessionData.scheduled_at).toLocaleTimeString('en-US', {
                        hour: 'numeric',
                        minute: '2-digit'
                      })}
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-3 bg-muted/50 rounded-lg p-3">
                      <div className="h-10 w-10 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-medium flex-shrink-0">
                        {demoSessionData.tutor_profiles.full_name?.charAt(0) || 'T'}
                      </div>
                      <span className="text-sm break-words">{demoSessionData.tutor_profiles.full_name}</span>
                    </div>
                    <div className="flex items-center gap-3 bg-muted/50 rounded-lg p-3">
                      <div className="h-10 w-10 rounded-full bg-secondary flex items-center justify-center text-secondary-foreground font-medium flex-shrink-0">
                        {demoSessionData.learner_profiles.full_name?.charAt(0) || 'L'}
                      </div>
                      <span className="text-sm break-words">{demoSessionData.learner_profiles.full_name}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 mt-6 border-t">
            ...
            <Button 
              ...
              className="w-full sm:w-auto rounded-xl order-2 sm:order-1"
            >
              Cancel
            </Button>
          </div>
